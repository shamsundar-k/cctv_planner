  ...
  return (
    <div className="flex items-start justify-between gap-2">
      <div className="min-w-0">
        <h3 className="text-sm font-semibold text-primary m-0 truncate">{camera.name}</h3>
        <p className="text-xs text-muted mt-0.5 m-0 truncate">
          {[camera.manufacturer, camera.model_number].filter(Boolean).join(' - ') || '—'}
        </p>
      </div>
      <span className="px-2 py-0.5 text-[11px] font-semibold rounded-full bg-surface text-muted">
