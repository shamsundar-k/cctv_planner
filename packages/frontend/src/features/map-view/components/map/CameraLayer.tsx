/**
 * CameraLayer — orchestrates per-camera markers.
 *
 * Subscribes only to `uids` from the Zustand store, so it re-renders
 * only when a camera is added or removed — never when a camera's properties change.
 */
  ...
  useEffect(() => {
    ...
    const handler = (e: LeafletMouseEvent) => {
      if (activeTool === 'place-camera' && selectedModelId) {
        const localCamera = generateDefaultCameraInstance(selectedModelId, { lat: e.latlng.lat, lng: e.latlng.lng }, projectId)
        if (localCamera) {
