...
export const useCameraInstanceStore = create<CameraInstanceStoreState>()(
  devtools(
    (set, get) => ({
      ...
      loadCameras: async (projectId) => {
        ...
        try {
          ...
          for (const camera of res.data) {
            clientIds.push(camera.client_id)
            cameraRecords[camera.client_id] = {
              camera,
      ...
      // Store uses camera.client_id as the stable key throughout the session

      addCamera: (camera) => {
        const clientId = camera.client_id
        if (!clientId) {
          console.error('addCamera: camera.client_id is required')
          return ''
        }
        set((s) => ({
      ...
      saveAll: async (projectId) => {
        ...
        for (const record of [...toPost, ...toPut]) {
          store.markSaving(record.camera.client_id)
        }
        ...
        await Promise.allSettled([
          ...
          ...toPost.map(async (record) => {
            const clientId = record.camera.client_id
            try {
              const payload: CameraInstanceCreatePayload = buildCreatePayload(record.camera)
              console.log("Sending post req for camera addition", payload.client_id)
